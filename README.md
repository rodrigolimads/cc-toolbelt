...
| `desk` | Spins up a parallel git worktree on its own branch so you can work on a second ticket without disturbing your primary checkout. Useful when, say, you're monitoring a long CI run on one branch and a hotfix lands. Accepts a Jira ID, a Sentry URL, or a free-text description. For Sentry and text inputs, opens a Jira ticket first (after a dedup search and preview) so the worktree has a ticket attached. Drops a `.desk-context.md` inside the worktree so the new Claude session opens with context already in hand. Reads each project's `.claude/config.md` for conventions, so the same skill works across repos. Companion commands: `/desk list` prints a table of all active desks with their tickets and branches; `/desk clean <jira-id>` tears down a desk (refuses to touch the primary checkout, asks before deleting the branch, and warns about uncommitted changes). |
